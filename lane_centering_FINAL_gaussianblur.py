...
def lane_centering():
    ...
    print("[INFO] Capturing and saving 40 processed images (0.25s apart)...")
    ...
    try:
        for i in range(40):
            ...
            # Invert: black lanes become white-like for better Canny
            gray = cv2.bitwise_not(gray)

            # Apply ROI
            masked = apply_roi_mask(gray)

            # Optional: Apply blur to reduce noise before edge detection
            ...
            if left_lines and right_lines:
                ...
                print(f"[{i+1}/40] Offset: {offset:.2f}px | θ: {theta_deg:.2f}° | Steering: {steering_angle:.2f}°")
            else:
                print(f"[{i+1}/40] No valid lanes detected.")
    ...
    cap.release()
    print("[INFO] Capture complete. Saved to /processed_images")
