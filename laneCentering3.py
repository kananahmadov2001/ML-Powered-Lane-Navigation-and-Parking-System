...
def draw_trajectory(image, lane_center_x, steering_angle):
    """Draws a trajectory line based on the predicted steering angle"""
    height, width, _ = image.shape
    car_center_x = width // 2
    start_point = (car_center_x, height - 50)  # Starting point near the bottom
    
    # Determine the end point by projecting the trajectory forward
    length = 150  # Adjust trajectory length
    angle_rad = np.radians(steering_angle)
    
    end_x = int(car_center_x + length * np.sin(angle_rad))
    end_y = int(height - 50 - length * np.cos(angle_rad))
    
    # Draw trajectory line
    cv2.arrowedLine(image, start_point, (end_x, end_y), (0, 0, 255), 3, tipLength=0.3)
    
    return image

def process_image(image_path):
    """Processes an image and estimates lane position and steering correction"""
    ...
    # Draw lane lines and lane center
    for x1, y1, x2, y2 in left_lines:
        cv2.line(image, (x1, y1), (x2, y2), (255, 0, 0), 3)
    for x1, y1, x2, y2 in right_lines:
        cv2.line(image, (x1, y1), (x2, y2), (0, 255, 0), 3)
    
    cv2.circle(image, (lane_center_x, height - 50), 10, (0, 255, 255), -1)

    # Draw trajectory based on steering angle
    image = draw_trajectory(image, lane_center_x, steering_angle)
    ...
    cv2.imshow("Lane Detection & Trajectory", image)
    cv2.waitKey(0)
...
# Example usage
process_image("image.jpg")  # Replace with your PiCamera image
